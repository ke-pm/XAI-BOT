...
class DialogTransformerDataset(data.Dataset):
    ...
    def __init__(self, file_path, tokenizer, 
                 min_num_utts=1, max_num_utts=3, max_utt_len=30, 
...
class ResponseGPTDataset(DialogTransformerDataset):
    """
    """
    def __init__(self, context, tokenizer, 
                 max_num_utts=5, max_utt_len=120,
                _fewshot=-1):
        self.tokenizer = tokenizer
        self.max_num_utts = max_num_utts
        self.max_utt_len=max_utt_len
    ...
    def __getitem__(self, _offset):
        context = []
        for uttr in self.context:
            context = context + self.tokenizer.encode(uttr) + [self.eos_token_id]
...
class DialogGPTDataset(DialogTransformerDataset):
    ...
    def __init__(self, file_path, tokenizer, 
        ...
        print('length of the data',len(self.data))
        self.index = table.get_node('/indices')[:]
        if fewshot>-1:
            self.index = self.index[:fewshot]
        self.data_len = self.index.shape[0]
        print('length of the data again ',len(self.data))
        print("{} entries".format(self.data_len))
    ...
    def __getitem__(self, offset):
        ...
        #print('context:'+ tokenizer.decode(self.data[pos_utt-ctx_len: pos_utt]))
        res_arr=self.data[pos_utt:pos_utt+res_len].tolist()
        #print('response:'+ tokenizer.decode(self.data[pos_utt:pos_utt+res_len]))
        ...
        input_ids = context + res
        #print('input id:', tokenizer.decode(context),tokenizer.decode(res))
        attn_mask = [1]*len(input_ids)
        lm_labels = [-100]*len(context) + res
