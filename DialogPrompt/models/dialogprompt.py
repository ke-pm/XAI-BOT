...
class DialogPrompt(nn.Module):
    ...
    @classmethod        
    def from_pretrained(self, model_dir, args):
        self.tokenizer = GPT2Tokenizer.from_pretrained(os.path.join(model_dir, 'tokenizer'))
        self.gpt = GPT2LMHeadModel.from_pretrained(os.path.join(model_dir, 'gpt'))
        self.gpt.resize_token_embeddings(len(self.tokenizer))
        base_model_name='gpt2-medium' if args.model_size =='medium' else 'gpt2'
        base_model_name ='gpt2-large' if args.model_size == 'large' else base_model_name
        base_model_name ='gpt2-xl' if args.model_size == 'xlarge' else base_model_name
    ...
    def save_pretrained(self, model_dir):   
        ...
        torch.save(model_to_save.state_dict(), os.path.join(model_dir, 'state_dict.bin'))
    ...
    def generate(self, input_batch, max_len=100, num_samples=1, mode='sample'): 
        """
