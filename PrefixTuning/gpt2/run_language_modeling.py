...
from transformers import (
    ...
    LineByLineDialogMultiWozDataset,
...
class DataTrainingArguments:
    ...
    finetuned_model_path: Optional[str] = field(
        default="/home/elhadrik/contrast_LM/transformers/examples/full/full/webnlgfinetune_n_20_act_cat_b=6-e"
    ...
    lowdata_token: Optional[str] = field(
        default='Dialog', metadata={"help": "The token to be prepended at initialization time. "}
    )
...
collator: Dict[str, torch.Tensor] = {}
def get_dataset(
    ...
    if args.line_by_line:
        ...
        if args.task_mode == 'embMatch':
            ...
            dataset = LineByLineDialogDataset(tokenizer=tokenizer, file_path=file_path,
                                                   lowdata_token = args.lowdata_token if('lowdata' in training_args.output_dir and finetune_mode) else None,
                                                    max_history_length = 300, max_response_length = 100, block_size=args.block_size,  bos_tok=tokenizer.bos_token,
                                                   eos_tok=tokenizer.eos_token)
        else:
            return LineByLineTextDataset(tokenizer=tokenizer, file_path=file_path, block_size=args.block_size)
...
def main():
    ...
    if model_args.tuning_mode == 'prefixtune': # prefixtune
        ...
        # print(model.config)
        # print(model)
        print('run_language_modelling line 788')
        for param in model.base_model.parameters():
            param.requires_grad = False
    ...
    if (model_args.tuning_mode == 'prefixtune'):
        ...
        print('line 981 run_language_modelling')
        trainer = Trainer(
            model=model,
            tokenizer=tokenizer,
            args=training_args,
    ...
    if training_args.do_eval:
        ...
        print('this is supposedly the evaluation of the training set', eval_output)
    ...
    """ if 'lowdata' in training_args.output_dir:
        ...
        data_args.eval_data_file = "/home/elhadrik/e2e_data/src1_valid.txt"
        eval_dataset = (
            get_dataset(data_args, tokenizer=tokenizer, evaluate=True, cache_dir=model_args.cache_dir,
                        training_args=training_args, finetune_mode=(model_args.tuning_mode == 'finetune'))
            if training_args.do_eval
