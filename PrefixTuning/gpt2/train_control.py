        ...
        if True:
            ...
            if self.lowdata and self.lowdata_token is not None:
                ...
                print('LOW DATA SETTING, UNDER PARAMETRIZATION 1, low_data_init=3, '
                      'preseqlen = {} Unifying with FINETUNE'.format(self.preseqlen),self.lowdata_token[0])
            ...
            # DIFFERENT PARAMETRIZATION:
                ...
                #create a tensor containing a sequence of integers from 0 to prefix_length-1
                self.input_tokens = torch.arange(self.preseqlen).long()
                # creation of prefix continuous vectors 
                self.wte = nn.Embedding(self.preseqlen, config.n_embd)
                self.control_trans = nn.Sequential(
                    nn.Linear(config.n_embd, self.mid_dim),
                    nn.Tanh(),
