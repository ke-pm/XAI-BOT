    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if (
                        ...
                        #print('inside the if for the steps')
                        metrics = self.evaluate()
                        #print('line 868', metrics)
                        ...
                        if 'lowdata' in self.args.output_dir or 'earlystop' in self.args.output_dir:
                            print('the earlystop is recognized')
                            self.save_based_on_eval = True
                        else:
                            print('the earlystop is not recognized')
                        ...
                        if self.save_based_on_eval and metrics["eval_loss"] < self.curr_best_eval:
                            ...
                            print('output_dir contains', self.args.output_dir, checkpoint_folder)
                            self.store_flos()
            ...
            if self.args.evaluation_strategy == EvaluationStrategy.EPOCH:
                #print('inside the if for the epochs')
                metrics = self.evaluate()
                self._report_to_hp_search(trial, epoch, metrics)
