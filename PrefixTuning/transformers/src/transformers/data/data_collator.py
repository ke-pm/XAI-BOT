...
class DataCollatorForText2DataLanguageModeling:
    ...
    def mask_tokens(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        return inputs, labels
@dataclass
class DataCollatorForDialogGeneration:
    """
    Custom data collator for dialog generation.
    - Collates batches of tensors, honoring the tokenizer's pad_token
    - Formats data for dialog generation
    """
    ...
    def __call__(
        ...
        if isinstance(examples[0], (dict, BatchEncoding)):
            examples = [e["input_ids"] for e in examples]
        # Assumes the input is a list of tuples where each tuple contains (history, response)
        input_ids, labels,  = zip(*examples)
        if self.mlm:
            inputs, labels = self.mask_tokens(input_ids)
            return {"input_ids": inputs, "labels": labels}
        # Format the batches based on the specified mode
        else :
            if self.format_mode == 'cat':
                ...
                raise ValueError(f"Unsupported format_mode: {self.format_mode}")
        ...
        # Attention masks
        # attn_mask = (batch != self.tokenizer.pad_token_id)
