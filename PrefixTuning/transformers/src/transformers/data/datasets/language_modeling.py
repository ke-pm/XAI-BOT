...
class LineByLineDialogMultiWozDataset(Dataset):
    def __init__(
        self, 
        tokenizer: PreTrainedTokenizer, 
        file_path: str, 
        ...
        for dialogues in data:
            for dialogue in dialogues:
                ...
                for i in range(1, len(utterances)):
                    history = utterances[max(i - max_history, 0):i]
                    joined_history = ' '.join(history)
                    response = utterances[i]
                    edited_sent = ' {} {} '.format(joined_history, self.bos_tok) + response + ' {}'.format(self.eos_tok)
                    self.examples.append(edited_sent) """
                # From previous run
                utterances = utterances[:4]
                response = utterances[-1]
                history = ' '.join(utterances[:-1])
                edited_sent = ' {} {} '.format(history, self.bos_tok) + response + ' {}'.format(self.eos_tok)
                self.examples.append(edited_sent)
        batch_encoding = self.tokenizer(self.examples, max_length=self.block_size, truncation=True, padding=True)
        self.examples = batch_encoding["input_ids"]
        self.labels = copy.deepcopy(self.examples)
    ...
    def __len__(self):
        return len(self.examples)
class LineByLineDialogDataset_old(Dataset):
    def __init__(
        self, 
        tokenizer: PreTrainedTokenizer, 
        file_path: str, 
        ...
        self.eos_tok = eos_tok
        self.block_size = block_size
        self.lowdata_token = lowdata_token
        with open(file_path, 'r', encoding='utf-8') as txt_file:
            if self.lowdata_token is None: 
                for line in txt_file:
                    ...
                    if len(dialogue) > 1:
                        ...
                        self.histories.append(history_str)
                ...
                for line in txt_file:
                    ...
                    if len(dialogue) > 1:
                        ...
                        edited_sent = '{} {} '.format(history_str, self.bos_tok) + response + ' {}'.format(self.eos_tok)
                        self.examples.append(edited_sent)
            batch_encoding = self.tokenizer(self.examples, max_length=self.block_size, truncation=True, padding=True)['input_ids']
            #print('batch_encoding', batch_encoding)
            self.labels = copy.deepcopy(batch_encoding)
            print('labels,', self.labels)
            histories = self.tokenizer(self.histories, max_length=self.max_history_length, truncation=True, padding=True)['input_ids']
            responses = self.tokenizer(self.responses, max_length=self.max_response_length, truncation=True, padding=True)['input_ids']
            bos_tokenized = self.tokenizer(self.bos_tok, add_special_tokens=False)['input_ids']
            #print('bos_tokenized', bos_tokenized)
            eos_tokenized = self.tokenizer(self.eos_tok, add_special_tokens=False)['input_ids']
            self.examples =[]
            for i in range(len(histories)):
                self.examples.append(histories[i] + bos_tokenized + responses[i] + eos_tokenized)
...
class LineByLineDialogDataset(Dataset):
            ...
            if self.lowdata_token is None: 
                ...
                for line in txt_file:
                    ...
                    if len(dialogue) > 1:
                        ...
                        history_str = ' '.join(history)
                        #print(len(history_str))
                        edited_sent = ' {} {} {} '.format(lowdata_token, history_str, self.bos_tok) + response + ' {}'.format(self.eos_tok)
                        self.examples.append(edited_sent)
            batch_encoding = self.tokenizer(self.examples, max_length=self.block_size, truncation=True, padding=True)
            self.examples = batch_encoding["input_ids"]
            self.labels = copy.deepcopy(self.examples)
    ...
    def __len__(self):
        return len(self.examples)
class LineByLineDialogDatasetupdated(Dataset):
    def __init__(
        self, 
        tokenizer: PreTrainedTokenizer, 
        file_path: str, 
        ...
        self.max_source_length = max_history_length
        ...
        self.max_target_length = max_response_length
    ...
    def load(self, data_file):
        with open(data_file, 'r', encoding='utf-8') as txt_file:
            for line in txt_file:
                dialogue = line.strip().split('__eou__')
                dialogue = [turn.strip() for turn in dialogue if turn.strip()]
                if len(dialogue) > 1:
                    history = dialogue[:-1]
                    response = dialogue[-1]
                    history =  f" {self.sep_token} ".join(history)
    ...
    def collate_fn(self, batch) -> Dict[str, torch.Tensor]:
        ...
        source_ids, source_mask = self.trim_batch(input_ids, pad_token_id, attention_mask=masks)
        y = self.trim_batch(target_ids, pad_token_id)


        bow_target_ids = torch.stack([x['bow_targets'] for x in batch])
        bow_y = self.trim_batch(bow_target_ids, pad_token_id)
